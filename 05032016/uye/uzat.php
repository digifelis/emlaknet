<TABLE border=0 cellSpacing=0 cellPadding=0 width="100%">
  ...
  <tr>
    <td width="99%">
      <?php
$vt->sql('select ilan_suresi from ilan_ticari where id = "'.temizle($_GET["id"]).'" and uye_id= "'.$_SESSION["id"].'" ')->sor();	
	if($vt->numRows() == 1) {
		$yeni_sure = $vt->alTek() + temizle($_POST["RadioGroup1"]) ;
	$vt->sql('update ilan_ticari set ilan_suresi = "'.$yeni_sure.'" , onay = 3  where id = "'.temizle($_GET["id"]).'" and uye_id= "'.$_SESSION["id"].'"  ')->sor();	
	if($vt->affRows() > 0) { echo "Ýlan Süresi Uzatýlmýþtýr"; } else {  echo "Ýþlemde Hata Oluþtu"; }	
