...
{
	echo ' 
	<A class=f2  href="index.php?action=mutahit_liste&amp;b='.$veri->plaka.'">'.$veri->sehiradiUpper.'</A><BR>
	';
}
 ?>
 ...
 foreach( $veriler as $veri )
{
 ...
 ?>
    ...
    <TD vAlign=top width=557>
      ...
      <div class="baslik_sol"></div>
      <div class="baslik_sag"><br />
        <span class="stil55">M&uuml;teahhit Listesi</span></div>
      </div>   
                       </TD>
          </TR>
        <TR>
          <TD>
            ...
            bgColor=#f7f7f7 align=center>
              ...
              <TR>
                <TD 
                style="BORDER-BOTTOM: #cccccc 1px solid; BORDER-LEFT: #cccccc 1px solid; BORDER-TOP: #cccccc 1px solid; BORDER-RIGHT: #cccccc 1px solid">
                  <DIV align=center><font face="Verdana">Türkiyenin Se&ccedil;kin m&uuml;teahhit firmalarýna 
                  ulaþmanýz artýk &ccedil;ok kolay !<BR><BR>
                  Farklý ilde bulunan 
                  m&uuml;teahhitler i&ccedil;in                   sol tarafdaki listeyi kullanabilirsiniz. 
                  <BR><BR>
                  <FONT color=#ff0000><STRONG><A 
                  style="COLOR: #ff0000; TEXT-DECORATION: none" 
                  title="Emlakçý Listesinde Yer Almak Ýstiyorsanýz Týklayýnýz." 
                  href="index.php?action=yeni_emlakci">M&uuml;teahhit Listesinde Yer 
		...
		/* SELECT uyeler.* , sehir.sehiradiUpper , count(ilan_ticari.id) as sayi  FROM  uyeler , sehir , ilan_ticari WHERE uyeler.il =  sehir.plaka and uyeler.uye_tipi=3 and ilan_ticari.uye_id = uyeler.id ".$kriter." group by uyeler.id order by uyeler.id desc */
		$kriter2="";
				   if(temizle($_GET["b"]) != "") { $kriter2 = "and uyeler.il = '".temizle($_GET["b"])."'"; }
				   $vt->sql("SELECT uyeler.* , sehir.sehiradiUpper , count(ilan_ticari.id) as sayi  FROM  uyeler , sehir , ilan_ticari WHERE uyeler.il =  sehir.plaka and uyeler.uye_tipi=3 and uyeler.onay=0 ".$kriter2." group by uyeler.id order by uyeler.id asc   ")->sor();
                    $veriler = $vt->alHepsi();
					foreach ($veriler as $veri) {
