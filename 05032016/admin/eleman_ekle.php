...
width=129>
     ...
     <?php $vt->sql("select * from ilan_ticari where id= '".temizle($_GET["id"])."' ")->sor();
	 $veriler = $vt->alHepsi();
	 foreach($veriler as $veri1) {
		 $uye_id = $veri1->uye_id;
                   ?>                     
                                  <?php 
 $vt->sql('select adi from ilan_tipi where id="'.$veri1->ilan_tipi_id.'"   ')->sor();
echo $vt->alTek()."&nbsp;-&nbsp;"; 

$vt->sql('select sehiradiUpper from sehir where sehirID="'.$veri1->il.'"   ')->sor();
echo $vt->alTek()."&nbsp;-&nbsp;"; 

$vt->sql('select ilceAdi from ilce where ilceID="'.$veri1->ilce.'" and sehirID="'.$veri1->il.'"  ')->sor();
echo $vt->alTek()."&nbsp;-&nbsp;"; 

$vt->sql('select mahalleAdi from mahalle where mahalleID="'.$veri1->koy.'" and ilceID="'.$veri1->ilce.'" and sehirID="'.$veri1->il.'"   ')->sor();
echo $vt->alTek(); 
 ?> 
  <?php } ?>                    
...
width=80><STRONG>Fotoðraf</STRONG></TD>
              ...
              <?php
			 $vt->sql("select * from bayi_eleman where uye_id = '".$uye_id."'")->sor();
			 $veriler = $vt->alHepsi();
			 foreach($veriler as $veri) {
                        ?>
                    <TR>
