...
$vt->sql("update duyuru set baslik= '".temizle($_POST["baslik"])."' , icerik = '".temizle($_POST["haber"])."' , gorunme = '".temizle($_POST["gorunme"])."' where id = '".temizle($_GET["id"])."' ")->sor();

 if($vt->affRows() > 0)
 { echo "<center>DUYURU BAÞARILI BÝR ÞEKÝLDE DÜZELTILDI.</center>"; } else {echo "<center> DUYURU DÜZELTMEDE HATA OLUÞTU LÜTFEN TEKRAR DENEYÝNÝZ!!!! </center> "; }
 
}

$vt->sql("select * from duyuru where id = '".temizle($_GET["id"])."'")->sor();
  $veriler = $vt->alHepsi();
