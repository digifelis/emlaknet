...
<div class="sol_item">
    ...
    <div class="sol_item_orta">   
        <form action="http://www.emlaknetturkiye.com/index.php?action=giris" method="post" style="margin-top:0px; margin-bottom:0px;">
            ...
            <p style="margin-top:2px;"><input name="kod" type="text" style="width:120px; height:16px;" /></p>
             <p style="margin-top:2px;"><label><img src="http://www.emlaknetturkiye.com/g_resim.php"></label></p>
...
FROM ilan_tipi, ilan_ticari where tip=1 and grup_id=1 and  ilan_tipi.id in (select distinct ilan_tipi_id  from ilan_ticari ) and uye_id="'.$_SESSION["emlakci"].'" ')->sor();
$veriler = $vt->alHepsi();
 foreach( $veriler as $veri )
{ echo ' <option value="'.$veri->id.'">'.$veri->adi.'</option>'; }

$vt->sql('SELECT distinct ilan_tipi.*
FROM ilan_tipi, ilan_ticari where tip=0 and grup_id=1 and ilan_tipi.id in (select distinct ilan_tipi_id  from ilan_ticari )  and uye_id="'.$_SESSION["emlakci"].'" ')->sor();
$veriler = $vt->alHepsi();
 foreach( $veriler as $veri )
{ echo ' <option value="'.$veri->id.'">'.$veri->adi.'</option>'; }

$vt->sql('SELECT distinct ilan_tipi.*
FROM ilan_tipi, ilan_ticari where tip=1 and grup_id=2 and ilan_tipi.id in (select distinct ilan_tipi_id  from ilan_ticari ) and uye_id="'.$_SESSION["emlakci"].'"  ')->sor();
$veriler = $vt->alHepsi();
 foreach( $veriler as $veri )
{ echo ' <option value="'.$veri->id.'">'.$veri->adi.'</option>'; }

$vt->sql('SELECT distinct ilan_tipi.*
FROM ilan_tipi, ilan_ticari where tip=0 and grup_id=2 and ilan_tipi.id in (select distinct ilan_tipi_id  from ilan_ticari ) and uye_id="'.$_SESSION["emlakci"].'" ')->sor();
$veriler = $vt->alHepsi();
 foreach( $veriler as $veri )
{ echo ' <option value="'.$veri->id.'">'.$veri->adi.'</option>'; }

$vt->sql('SELECT distinct ilan_tipi.*
FROM ilan_tipi, ilan_ticari where tip=1 and grup_id=3 and ilan_tipi.id in (select distinct ilan_tipi_id  from ilan_ticari )  and uye_id="'.$_SESSION["emlakci"].'" ')->sor();
$veriler = $vt->alHepsi();
 foreach( $veriler as $veri )
{ echo ' <option value="'.$veri->id.'">'.$veri->adi.'</option>'; }

$vt->sql('SELECT distinct ilan_tipi.*
FROM ilan_tipi, ilan_ticari where tip=0 and grup_id=3 and ilan_tipi.id in (select distinct ilan_tipi_id  from ilan_ticari )  and uye_id="'.$_SESSION["emlakci"].'"  ')->sor();
$veriler = $vt->alHepsi();
 foreach( $veriler as $veri )
{ echo ' <option value="'.$veri->id.'">'.$veri->adi.'</option>'; }
 ?>
 ...
 <input name="resim" type="hidden" id="action" value="0" />                 
  <input name="sira" type="hidden" id="action" value="1" /> 
            ...
            <input name="fiyat_ust" type="text"   onKeyPress="return numbersonly(this, event)" size="10" maxlength="10" />
                    ...
                    <div style="float:right; margin-top:15px; margin-right:10px;"><input type="image" src="res/ara.png" /></div>
...
src="http://pagead2.googlesyndication.com/pagead/show_ads.js">
            ...
            <div class="orta_item">
                ...
                <?php 
  		$vt->sql("select id from ilan_ticari where onay='4' and uye_id='".$_SESSION["emlakci"]."' ")->sor();
				$veriler = $vt->alHepsi();
				foreach($veriler as $veri) {
				$rastgele[]= $veri->id;
				}
...
if($vt->alTek() != "")
{ 
$vitrin_resim = '<img name=""  src="thumb.php?p=resimler/'.$vt->alTek().'&w=100&h=75" alt="" width="120px" height="90px" border="0" />';
} else { 
$vitrin_resim =  '<img  class="photo"  border=0 alt="'.$ilan_basligi.'" src="http://'.$site.'/thumb.php?p=resimler/nopic100x60.gif&w=100&h=75" width="120px" height="90px" border="0">'; 
}	
										}
...
{ $vitrin_resim = '<IMG class="photo"  border=0 alt="'.$ilan_basligi.'" src="http://'.$site.'/thumb.php?p=resimler/'.$foto_adi.'&w=100&h=75" width="120px" height="90px" border="0">'; 
}
...
$tablo  = '<div class="ilanitem">
							...
							<div class="ilanitem_foto"><a href="detail_'.$veri->id.'_'.$ilan_adres.'">'.$vitrin_resim.'</a></div>
							<div class="ilanitem_detay">
								
								<p><b>Þehir : </b>'.$ilan_il.'</p>
								<p><b>Þehir : </b>'.$ilan_ilce.'</p>
								<p><b>Fiyat : </b>'.ondalik($veri->fiyat)."&nbsp;".$birim.'</p>
								
								<p align="right">
									<a href="detail_'.$veri->id.'_'.$ilan_adres.'" style="color:#333; text-decoration:underline;">Detaylý Ýncele</a>
								</p>
							</div>
						</div>'; 
...
$vt->sql("select count(id)  from ilan_ticari where   onay != '1' and onay !='3' and uye_id='".$_SESSION["emlakci"]."'  ")->sor();

if($vt->alTek()<=$yeni_ilan_sayisi){ $yeni_ilan_sayisi = $vt->alTek(); }
...
// $vt->sql("select * from ilan_ticari where onay !='1' and onay !='3' order by id DESC limit ".$ii.",1 ")->sor();	

$vt->sql("select ilan_ticari.id as id,ilan_ticari.birim,ilan_ticari.fiyat , ilan_ticari.uye_id , ilan_tipi.adi , sehir.sehiradiUpper , ilce.ilceAdi , mahalle.mahalleAdi , uyeler.uye_tipi  from ilan_ticari , ilan_tipi , sehir , ilce , mahalle , uyeler   where   ilan_tipi.id=ilan_ticari.ilan_tipi_id and sehir.sehirID=ilan_ticari.il and ilce.ilceID=ilan_ticari.ilce and ilce.sehirID=ilan_ticari.il and mahalle.mahalleID=ilan_ticari.koy and mahalle.ilceID=ilan_ticari.ilce and mahalle.sehirID=ilan_ticari.il and uyeler.id=ilan_ticari.uye_id  and  ilan_ticari.onay !='1' and ilan_ticari.onay !='3'  and uyeler.id='".$_SESSION["emlakci"]."' group by ilan_ticari.id order by ilan_ticari.id  DESC limit ".$ii.",1")->sor();
...
if($vt->alTek() != "")
{ 
$ilan_resim = '<img name=""  src="http://'.$site.'/thumb.php?p=resimler/'.$vt->alTek().'&w=100&h=75" alt="" width="120px" height="90px" border="0" />';
} else { 
$ilan_resim =  '<img  class="photo"  border=0 alt="'.$ilan_basligi.'" src="http://'.$site.'/thumb.php?p=resimler/nopic100x60.gif&w=100&h=75" width="120px" height="90px" border="0">'; 
}	
										}
...
{ $ilan_resim = '<IMG class="photo"  border=0 alt="'.$ilan_basligi.'" src="http://'.$site.'/thumb.php?p=resimler/'.$foto_adi.'&w=100&h=75" width="120px" height="90px" border="0">'; 
}
