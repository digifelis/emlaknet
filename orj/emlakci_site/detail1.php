 ...
 <TR>
  ...
  if($vt->numRows() > 0) {  
   foreach($veriler as $veri) {
	 ...
	 echo '  
  <img itemprop="image" src="http://'.$site.'/thumb.php?p=resimler/'.$fotomuz.'&w=510&h=400"  name="resim" width="510" height="400"   /> ';  
	 } 
  ...
  $vt->sql("select * from logolar where user_id = '".$veri1->uye_id."'")->sor();
			 ...
			 foreach($veriler as $veri) {
				 if($veri->firma_logo != "") {
			 echo '<img itemprop="image" src="http://'.$site.'/resimler/'.$veri->firma_logo.'" width="510" height="400"  />';
				 }
				 }	  
  }
...
/* emlaknet turkiye -konut ilan sayfalari */
...
<IMG  border=0 src="http://'.$site.'/thumb.php?p=resimler/'.$veri->foto_adi.'&w=85&h=64"  onmouseover="resim.src=\'http://'.$site.'/thumb.php?p=resimler/'.$veri->foto_adi.'&w=510&h=400\'" >
