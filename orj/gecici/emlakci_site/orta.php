...
$vt->sql('SELECT ilan_ticari.* , uyeler.uye_tipi as uye_tipi , uyeler.sirket as sirket,  count(ilan_foto.ilan_id) as fotosay , ilan_tipi.tip as tip , ilan_tipi.grup_id FROM ilan_ticari, uyeler , ilan_foto,ilan_tipi where ilan_ticari.uye_id = uyeler.id and ilan_ticari.ilan_tipi_id=ilan_tipi.id     '.$_SESSION["kriter"].' and uyeler.id="'.$_SESSION["emlakci"].'"   group by ilan_ticari.id   order by ilan_ticari.onay DESC ,ilan_ticari.id DESC    '. $limit .'')->sor();
...
$yazdir = ' <TABLE border=0 cellSpacing=1 cellPadding=1 width="100%" 
  ...
  <TD height=20 vAlign=top width="75%" class=tdliste><FONT 
                ...
                <TD vAlign=top rowSpan=2 width=85 class=tdliste noWrap>
                  <DIV align=center><FONT color=#003399><b>
				  '. ondalik($veri->fiyat).' '.
				 $gbirim
					 .'
                  </FONT><h5>Ýlan 
                  Tarihi:<br>'. date("d-m-Y",strtotime($veri->bas_tarihi)) .'</h5> </DIV></TD></TR>
              <TR>
